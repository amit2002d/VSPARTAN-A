...
res = st.session_state.all_data["EASEMYTRIP"]
...
if 'Price' in st.session_state.res.columns and 'Qty.' in st.session_state.res.columns:
    st.session_state.res['Price'] = st.session_state.res['Price'].str.replace(',', '').astype(
        float) if st.session_state.res['Price'].dtype == 'object' else st.session_state.res['Price']
    st.session_state.res['Qty.'] = st.session_state.res['Qty.'].str.replace(',', '').astype(
        float) if st.session_state.res['Qty.'].dtype == 'object' else st.session_state.res['Qty.']
    st.session_state.res['Buy Value'] = st.session_state.res['Price'] * \
        st.session_state.res['Qty.']
    st.session_state.res['Age'] = (
        datetime.datetime.now() - pd.to_datetime(res['Date'])).dt.days
else:
    st.error("Columns 'Price' and/or 'Qty.' not found in the DataFrame.")

while True:
    df = pd.DataFrame(
        columns=['Total Investment', 'Current Value', 'ROI', "AVG Price", "Qty", 'PnL'])
    if time.time() - st.session_state.last_analysis_time >= 100:
        st.session_state.last_analysis_time = time.time()

        st.session_state.res['CMP'] = round(get_cmp_price(
            secrets["connections"]["gsheets"]["worksheets"]['EASEMYTRIP']), 2)
        st.session_state.res['Current Value'] = st.session_state.res['Qty.'] * \
            st.session_state.res['CMP']
        st.session_state.res['Gain%'] = round(
            ((res['Current Value'] - st.session_state.res['Buy Value']) / st.session_state.res['Buy Value']) * 100, 2)
        st.session_state.res['Amount'] = st.session_state.res['Current Value'] - \
            st.session_state.res['Buy Value']

        # st.session_state.res = res
        title.title('')
        title.title(f'Data for EASEMYTRIP')
        res_place.text('')
        res_rounded = st.session_state.res.round(2)
        format_dict1 = {'Total Investment': '{:.2f}', 'Current Value': '{:.2f}',
                        'ROI': '{:.2f}', "AVG Price": '{:.2f}', "Qty": '{:.2f}', 'PnL': '{:.0f}'}
        ...
        df = pd.DataFrame({'Total Investment': [buy_value], 'Current Value': [current_value], 'ROI': [
                          roi], "AVG Price": [avg_price], 'Qty': [total_qty], 'PnL': [gain]})
        styled_df = df.style.format(format_dict1).apply(
            highlight_condition, axis=0)
        styled_res = res_rounded.sort_values('Date').style.format(
            format_dict2).apply(highlight_gain_condition, subset=['Gain%'], axis=0)
