...
res = st.session_state.all_data["ANGELONE"]
...
while True:
    ...
    if time.time() - st.session_state.last_analysis_time >= 100:
        ...
        st.session_state.res['CMP'] = round(get_cmp_price(secrets["connections"]["gsheets"]["worksheets"]['ANGELONE']),2)
        ...
        # st.session_state.res = res
        title.title('')
        title.title(f'Data for ANGELONE')
        res_place.text('')
