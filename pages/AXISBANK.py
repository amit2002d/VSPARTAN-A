...
res = st.session_state.all_data["AXISBANK"]
...
while True:
    ...
    if time.time() - st.session_state.last_analysis_time >= 100:
        ...
        st.session_state.res['CMP'] = round(get_cmp_price(secrets["connections"]["gsheets"]["worksheets"]['AXISBANK']),2)
        ...
        # st.session_state.res = res
        title.title('')
        title.title(f'Data for AXISBANK')
        res_place.text('')
